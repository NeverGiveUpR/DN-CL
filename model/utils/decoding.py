...
class Search:
    ...
    def batch_decode(self, points: tf.Tensor, const_opt_points: tf.Tensor, use_clean_points_opt_const: bool = True):
        raise NotImplementedError()
...
class RandomSampler(Search):
    ...
    def encoder_feature(self, points: tf.Tensor):
        encoder_state = self.model.encoder(points, False)
        # feature = self.model.projection_layer(tf.reduce_mean(encoder_state, axis=1))
        feature = tf.reduce_mean(encoder_state, axis=1)
        return feature

    def batch_decode(self, points: tf.Tensor, # noisy data points
                     const_opt_points: tf.Tensor, # clean data points
                     use_clean_points_opt_const: bool = True, 
        ...
        for i in range(tf.shape(points)[0]):
            ...
            (
                ...
                preds, reg_preds, points[i], const_opt_points[i], logits, return_all=return_all,
                use_clean_points_opt_const=use_clean_points_opt_const
            )
...
class GreedySearch(Search):
    def batch_decode(self, points: tf.Tensor, const_opt_points: tf.Tensor, use_clean_points_opt_const: bool = True):
        ...
        if self.reg_input:
            if use_clean_points_opt_const:
                return self.convertor.convert(preds, reg_preds, points, const_opt_points, logits)
            else:
                return self.convertor.convert(preds, reg_preds, points, points, logits)
        else:
            if use_clean_points_opt_const:
                return self.convertor.convert(preds, None, points, const_opt_points, logits)
            else:
                return self.convertor.convert(preds, None, points, points, logits)

    @tf.function
    def decode(self, points: tf.Tensor):
        tf.print(tf.shape(points))
        ...
        print("logits_array:", logits_array.shape)
        ...
        print("regression_output_array:", regression_output_array.shape)

        all_predicted = tf.zeros(tf.shape(start), dtype=tf.bool)
        print("all_predicted:", all_predicted.shape)
        ...
        for i in tf.range(self.max_len):
            ...
            # tf.print(tf.reduce_all(all_predicted))
            if tf.reduce_all(all_predicted):
                print("break from the list")
                break
        return (
