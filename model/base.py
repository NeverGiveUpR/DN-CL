...
class TransformerBase(tf.keras.Model):
    def __init__(self, cfg: Config, tokenizer, strategy: tf.distribute.Strategy):
        ...
        self.h_metrics = {
            ...
            "contrastive_loss": tf.keras.metrics.Mean(name="contrastive_loss"),
    ...
    def contrastive_loss(self, inp_feature, inp_noise_feature, t=0.07):
        # batch x d_model
        feature_dim = tf.shape(inp_feature)[-1]
        feature_dim_sqrt = tf.sqrt(tf.cast(feature_dim, tf.float32))

        logits = tf.matmul(inp_feature, tf.transpose(inp_noise_feature)) / feature_dim_sqrt
        labels = tf.linalg.diag(tf.ones(tf.shape(logits)[0]))
        loss = tf.keras.losses.categorical_crossentropy(labels, logits/t, from_logits=True)
        return loss
    ...
    def update_metrics(
        ...
        if contrastive_loss == None:
            self.h_metrics['contrastive_loss'](self.normalize_loss(tf.zeros_like(regression_loss)))
        else:
            self.h_metrics['contrastive_loss'](self.normalize_loss(contrastive_loss))
