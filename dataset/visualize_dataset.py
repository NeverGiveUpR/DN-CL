...
def visualize(func, points, func_str, i):
    ...
    plt.savefig(f"/home/liujingyi/symformer-main-noise/symformer-main/data/visualisations/{i}.png")
    plt.clf()
    plt.close()
    return True
...
def check_if_nan(dataset):
    ...
    for data in dataset.as_numpy_iterator():
        ...
        success = np.isnan(np.sum(data["points"]))
        print("i:", j, "success:", success)
        print("func_str:", func_str)
        print("func:", func)
        if success:
            print("{} contains NaN.".format(func_str))
            print(data["points"])
        j += 1
        if j == 10:
            print(data["points"])
...
def main(path: str, /, num: int = 200):
    ...
    """
    path = '/home/liujingyi/symformer-main-noise/symformer-main/data/train/'
    dataset = load_dataset(path).take(num * 10)
    # vizualize_dataset(dataset)
    check_if_nan(dataset)
